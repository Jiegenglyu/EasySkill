...
function assertExpected(step, result) {
  ...
  for (const expected of asList(step.expect?.contentTypeIncludes)) {
  ...
  for (const rejected of asList(step.expect?.rejectContentTypes || step.output?.rejectContentTypes)) {
